...
{
    internal class NMCHeader
    {
        // Fields
        private static readonly ushort[] PLAT_CRC16_TBL = new ushort[] {
        ...
        // Methods
        public static byte[] ComputeRhodesChecksum(byte[] bytes)
        {
            ushort index = 0;
            ushort num2 = 0;
            foreach (byte num3 in bytes)
            {
                index = (ushort)(((num2 >> 8) ^ num3) & 0xff);
                num2 = (ushort)((num2 << 8) ^ PLAT_CRC16_TBL[index]);
            }
            return BitConverter.GetBytes((int)((short)(num2 & 0xffff)));
        }

        public byte[] CreateByteArray(bool bIncludeHeaderCrc)
        {
            List<byte> list = new List<byte>();
            list.AddRange(BitConverter.GetBytes(this.headerVersion));
            list.AddRange(BitConverter.GetBytes(this.appVersion));
            list.AddRange(Encoding.ASCII.GetBytes(this.appVersionStr));
            list.AddRange(BitConverter.GetBytes(this.fileVersion));
            list.AddRange(BitConverter.GetBytes(this.fileType));
            list.AddRange(BitConverter.GetBytes(this.fileSize));
            list.AddRange(BitConverter.GetBytes(this.fileCrc));
            if (bIncludeHeaderCrc)
            {
                list.AddRange(BitConverter.GetBytes(this.headerCrc));
            }
            return list.ToArray();
        }
        ...
        public char[] appVersionStr { get; set; }
